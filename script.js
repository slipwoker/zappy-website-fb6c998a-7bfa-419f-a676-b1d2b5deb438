  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, rule, ruleValue) {
      const label = field.closest('.form-group, .field-wrapper, label')
        ? (field.closest('.form-group, .field-wrapper') || field.parentElement).querySelector('label')
        : null;
    ...
    function validateField(field) {
      ...
      if (field.hasAttribute('required')) rules.push(['required', null]);
      if (type === 'email') rules.push(['email', null]);
      if (type === 'tel') rules.push(['phone', null]);
      if (field.hasAttribute('minlength')) rules.push(['minLength', field.getAttribute('minlength')]);

      for (let i = 0; i < rules.length; i++) {
        const rule = rules[i][0];
        const ruleValue = rules[i][1];

        if (rule === 'required' && !VALIDATORS.required(value)) {
          showError(field, getErrorMessage(field, rule, ruleValue));
          return false;
        }
    ...
    const staggerObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          const staggerDelay = parseInt(container.getAttribute('data-stagger') || '100', 10);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "ליגת החלב משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'fb6c998a-7bfa-419f-a676-b1d2b5deb438',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'fb6c998a-7bfa-419f-a676-b1d2b5deb438';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'fb6c998a-7bfa-419f-a676-b1d2b5deb438';
